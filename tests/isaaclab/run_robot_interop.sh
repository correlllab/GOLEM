...
trap cleanup EXIT
"${COMPOSE[@]}" run -d --no-deps --name "$subscriber" -v "$ROOT/tests:/home/code/tests:ro" --entrypoint bash ros /home/code/tests/isaaclab/subscribe_robot_interop.sh >/dev/null
...
"${COMPOSE[@]}" run -d --no-deps --name "$simulator" --entrypoint bash isaac /home/code/h12_sim_scripts/launch_isaac.sh --headless --max_steps 400 >/dev/null
