...
if ! /usr/bin/python3 -c 'from unitree_hg.msg import LowCmd, LowState; from unitree_go.msg import MotorCmds, MotorStates; from livox_ros_driver2.msg import CustomMsg' >/dev/null 2>&1; then
    workspace=$(mktemp -d)
    for package in unitree_hg unitree_go livox_ros_driver2; do
...
CMAKE
    ...
    cd "$workspace"
    colcon build --packages-select unitree_hg unitree_go livox_ros_driver2 --cmake-args -DPython3_EXECUTABLE=/usr/bin/python3
